...
class oriResize(object):
    # Resize the input to the given size, 'size' is a 2-element tuple or list in the order of (h, w).
    def __init__(self, size):
        self.size = size
        print('Using oriResize...')
    ...
    def __call__(self, image, label):
        ...
        def find_new_hw(ori_h, ori_w):
            new_h = ori_h
            new_w = ori_w

            if new_h % 8 != 0:
                new_h = (int(new_h /8)+1)*8

            if new_w % 8 != 0:
                new_w = (int(new_w /8)+1)*8
 
            return new_h+1, new_w+1           

        test_size = self.size
        new_h, new_w = find_new_hw(image.shape[0], image.shape[1])
        image_crop = cv2.resize(image, dsize=(int(new_w), int(new_h)), interpolation=cv2.INTER_LINEAR)
        image = image_crop

        s_mask = label
        new_h, new_w = find_new_hw(s_mask.shape[0], s_mask.shape[1])
...
class myResize(object):
    ...
    def __call__(self, image, label):
        ...
        s_mask = cv2.resize(s_mask.astype(np.float32), dsize=(int(new_w), int(new_h)),interpolation=cv2.INTER_NEAREST)
        back_crop_s_mask = np.zeros((test_size, test_size))
